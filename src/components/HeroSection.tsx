  ...
  return (
    <section className="relative min-h-[90vh] flex flex-col items-center justify-center overflow-hidden">
      ...
      <div className="absolute inset-0 -z-10 overflow-hidden">
        <div className="absolute top-1/4 -left-12 w-64 h-64 bg-primary/10 rounded-full filter blur-3xl opacity-60 animate-float" style={{ animationDelay: '0s' }}></div>
        <div className="absolute bottom-1/4 -right-12 w-72 h-72 bg-blue-300/20 rounded-full filter blur-3xl opacity-60 animate-float" style={{ animationDelay: '1s' }}></div>
        <div className="absolute top-3/4 left-1/3 w-48 h-48 bg-purple-300/20 rounded-full filter blur-3xl opacity-60 animate-float" style={{ animationDelay: '2s' }}></div>
      </div>

      <div className="container px-6 md:px-8 mx-auto">
        <div className="flex flex-col items-center text-center max-w-3xl mx-auto">
          <span className="glass px-4 py-1.5 rounded-full text-xs font-medium uppercase tracking-wider mb-6 animate-fade-in">
            The #1 Marketplace for College Students
          </span>
          ...
          <div className="mt-16 flex items-center gap-6 animate-fade-in" style={{ animationDelay: '300ms' }}>
            <div className="flex -space-x-2">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="w-8 h-8 rounded-full border-2 border-white bg-primary/10" />
              ))}
            </div>
